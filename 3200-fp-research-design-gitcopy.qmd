...
Extensive research on economic policy in Brazil presents a clear consensus and points us to the most appropriate setting for the application of synthetic controls. Several different authors employing a myriad of methods (@abreu, @barbpessoa, @borges2016, @pessoa2017, among others) affirm that a major shift in economic policy (from a macro, micro, monetary, fiscal, and public policy perspectives) happened in early 2006, when a new Finance Minister was appointed by the presidential administration. The shift was so prominent that both the government and the researchers named it as the New Economic Matrix (NEM). 

I will provide details of what were the specific policies within the New Economic Matrix in my final submission.

## 2- Hypothesis

My main hypothesis is that we will observe a greater underperfomance of Brazil's GDP per capita trend in comparison to the counterfactual (the synthetic control) than the 12% found in @decada. This hypothesis is based on three main arguments. 
...
Third, building upon the previous two arguments, it has been shown that the NEM had negative effects in the Brazilian economy. @borges2017 uses three different methodological approaches to argue that – both domestic and external – factors exogenous to the Brazilian economy are responsible for approximately 50% of the economic downturn in the 2012 - 2016 period. While he demonstrates support for some policies packaged within the NEM and argues that the deceleration in potential growth can be tied to several factors that are exogenous to (independent of) economic policy, the author accepts that most of the remaining 50% are caused by domestic economic policy mistakes of the NEM (not **all** of the remaining 50%, since the author leaves room for measurement errors there).
...
I created two preliminary figures that trace Brazilian GDP per Capita in comparison with selected countries. The goal of this preliminary visualization exercise is twofold. One, to investigate whether Brazil actually took off in the early 2000s, as the 2009 cover for The Economist affirmed [@brtakesoff]. Two, to make the case for why the synthetic control method is the best empirical tool available to help me shed light upon my research question.
...
setwd("/Users/rodrigo/Desktop/Spring 2024/ECON4900/CODING WORK")

data.raw <- read.csv("Data.raw.csv")

#datasets to be merged with the cleaned version of the one above:
data.ttrade <- read.csv("Indonesia terms of trade.csv")
#figure how to download xls debt file
data.debt <- read_excel("IMF DEBT.xls")
...
#7.2 Adding updated debt data
data.debt.step7 <- data.debt[-1,]
data.debt.step7 <- data.debt.step7[c(-22,-23),]

data.debt.step7 <- data.debt.step7 %>%
  rename(country = `General Government Debt (Percent of GDP)`)

#renaming country column
data.debt.step7$country[1] <- "AUS"
data.debt.step7$country[2] <- "BRA"
data.debt.step7$country[3] <- "BGR"
data.debt.step7$country[4] <- "CHL"
data.debt.step7$country[5] <- "CHN"
data.debt.step7$country[6] <- "COL"
data.debt.step7$country[7] <- "CRI"
data.debt.step7$country[8] <- "EGY"
data.debt.step7$country[9] <- "HUN"
data.debt.step7$country[10] <- "IND"
data.debt.step7$country[11] <- "IDN"
data.debt.step7$country[12] <- "MYS"
data.debt.step7$country[13] <- "PER"
data.debt.step7$country[14] <- "PHL"
data.debt.step7$country[15] <- "POL"
data.debt.step7$country[16] <- "ROU"
data.debt.step7$country[17] <- "RUS"
data.debt.step7$country[18] <- "THA"
data.debt.step7$country[19] <- "TUR"
data.debt.step7$country[20] <- "UKR"
data.debt.step7$country[21] <- "VEN"

#making debt data long
data.debt.step7 <- data.debt.step7 %>%
  mutate_at(vars(-country), as.numeric)

debt.long <- data.debt.step7 %>%
...
data.scm.v4.merg <- data.scm.v4.vars.step7 %>% 
  left_join(debt.long, by = c("country", "year"), suffix = c("_vars", "_debt")) %>%
  mutate(debt = ifelse(is.na(debt_vars), debt_debt, debt_vars)) %>%
  select(-debt_debt, -debt_vars)
...
data.intro.graphs <- data.scm.v4.merg %>% 
